...
public class ApiSource {
	...
	/**
	 * Java classes containing Swagger's annotation <code>@Api</code>, or Java
	 * packages containing those classes can be configured here.
	 */
	...
	/*
	 * The transfer protocols of the API. Values MUST be from the list: "http",
	 * "https", "ws", "wss"
	 */
	private List<String> schemes;

	/**
	 * <code>templatePath</code> is the path of a hbs template file, see more
	 * details in next section. If you don't want to generate extra api
	 * documents, just don't set it.
	 */
	@Parameter
	...
	/**
	 * <code>attachSwaggerArtifact</code> triggers plugin execution to attach
	 * the generated swagger.json to Maven session for deployment purpose. The
	 * attached classifier is the directory name of
	 * <code>swaggerDirectory</code>
	 */
	@Parameter
	...
	private Contact from(io.swagger.annotations.Contact contactAnnotation) {
		return new Contact().name(contactAnnotation.name()).email(contactAnnotation.email())
				.url(contactAnnotation.url());
	}

	private License from(io.swagger.annotations.License licenseAnnotation) {
		return new License().name(licenseAnnotation.name()).url(licenseAnnotation.url());
	}
	...
	public boolean isWplexServices() {
		return wplexServices;
	}

	public void setWplexServices(boolean wplexServices) {
		this.wplexServices = wplexServices;
	}
