...
public class ApiDocumentMojo extends AbstractMojo {
	...
	/**
	 * A set of apiSources. One apiSource can be considered as a set of APIs for
	 * one apiVersion in a basePath
	 */
	@Parameter
		...
		try {
			...
			for (ApiSource apiSource : apiSources) {
				...
				if (apiSource.isWplexServices()) {
					documentSource = new WplexMavenDocumentSource(apiSource, getLog(), projectEncoding);
				} else {
